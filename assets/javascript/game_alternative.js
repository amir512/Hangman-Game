...
document.getElementById("startButton").addEventListener("click", function() {
		...
		var guess = event.key; //Define a variable to capture the user's guess
			...
			createHangmanSong(songs);
			document.onkeypress = function(event) {
  				var guess = event.key;
  				var answerArray = stringrandomSongNoCommas.map(i => '_');
  				
  				var i = 0;

				while (arrayrandomSong.indexOf(guess, i) != -1) {
					//replace letter in blank array[]
